...
| UX | 45 | **55** | anchored zoom 误差 10⁻¹³ px (commit e913fa1)；origin-shift threshold rebaseline 3× 抑制；**+ B-15 keyboard shortcut registry (commit b-15)** — `packages/core/src/input/`: combo parser + canonicalise (modifier-order-independent), 平台 `Mod` 解析 (mac→Meta, other→Ctrl), `createShortcutRegistry()` signal-based with conflict detection, `findByKeyboardEvent()` 接 DOM event 一行接入，37 tests covering parser edge cases / canonical order / conflict throws / platform Mod / `when` predicate / dispose | 真实 demo 渲染 + gesture recognizer + crosshair sync |
