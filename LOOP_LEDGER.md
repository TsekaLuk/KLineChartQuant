...
| 生态 | 65 | **75** | 21 个 framework binding (7×3) + sideEffects scope 修 + core 14 个 subpath exports + workspace:^ + LICENSE × 5 (commits c44f9a6, 291c4c4, 62d9dbb)；**+ B-23 versioning story (commit b-23)** — `CHANGELOG.md` × 5 publishable packages (Keep-a-Changelog format; `[Unreleased]` 段记录 foundation-drop 内容 + per-package highlights + peer-dep ranges), `.changeset/config.json` 锁定 linked 5 包 + ignored ui-schema + public access + main baseBranch（config 先于 CLI 进 repo，后续贡献者可 cleanly add `@changesets/cli`），`.changeset/README.md` 文档化 workflow + 决策依据，`.changeset/foundation-drop.md` minor-bump 5 包 with summary | 真 dist + publint 验证 + 安装 @changesets/cli + release CI workflow |
...
| meta | 2026-05-29 13:00 | **PR workflow decision recorded** — maintainer chose option 1 (accept #23 + #24 as foundation drop; tick 26+ post-merge = genuinely small PRs off main). PR #24 body got "How to land" addendum; PR_SPLIT_GUIDE.md rewritten as 2-phase workflow (Phase A pre-merge = continue on feat/renderer-interface; Phase B post-merge = branch off main per code-tick). **No scoreboard change** — process correction. | (push only) |
| 26 | 2026-05-29 13:45 | **B-23 versioning story** — 生态 axis (last sub-70 axis @ 65). Ships, in Phase A on `feat/renderer-interface`: `CHANGELOG.md` × 5 publishable packages in Keep-a-Changelog format with `[Unreleased]` capturing the foundation-drop scope + per-package highlights + peer-dep ranges; `.changeset/config.json` locking the structural decisions (5 linked publishable packages, ignored `ui-schema`, public access, `main` base branch, draft-3.0.0 schema) without installing `@changesets/cli` (deferred to a tick where install is needed); `.changeset/README.md` documenting workflow + rationale; `.changeset/foundation-drop.md` minor-bump changeset covering all 5 packages with cross-axis summary. Tests still 909 + 1 todo (docs-only). **生态 65→75**. **All 9 axes now ≥ 70.** Mean 72.3 → 73.4. Tick 27 candidates: AI-Native expansion (provider adapter), 美学/兼容性/AI-Native 都 @ 70 — wider field. | b-23 |
