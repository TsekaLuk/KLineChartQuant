...
| DX | 60 | **64** | 4 publishable packages + READMEs + LICENSEs + tsconfig.build × 5 + ai-runtime；DX audit 9 BLOCKER closed 6；KLineChartError base (b-3) + **b-3b 30 additional migrations** (12 indicators + 12 heatmap + ai-runtime) — 44/54 throw sites now typed (81%) | npm install reality + 真 dist + 余下 10 throws (MTF/originShift/AVWAP-types) 收口 |
| API | 62 | **70** | 5 包 contract test 绿；dispose silent no-op 统一；intake 动词全栈统一 (b-4 + b-4b)；BLOCKER-002 export * 8 内部 helper @internal；**BLOCKER-005 substantially closed** — 44/54 throws migrated, 13 KLineChartErrorCode (3 new: INDICATOR_INVALID_PARAM, HEATMAP_CONFIG_INVALID), ChartSerializationError extends shared base, cross-package `instanceof` works | canonical Bar、剩余 10 throws (B-3c)、return convention、runtime export 实际收口 |
