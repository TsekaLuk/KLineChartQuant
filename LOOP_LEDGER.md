...
| 31 | 2026-05-29 19:25 | **B-27 vitest babel decorator transform** — fixes env regression flagged at TRIBUNAL #6. Upstream added TC39 stage-3 decorators (`@Indicator(...)`) to legacy `src/core/*`. Bisection of `@klinechart-quant/core` barrel confirmed root cause: removing `createChartController` re-export → tests passed; restoring → reproduced. Applied vite-plugin-babel transform in all 5 vitest configs scoped to `include: [/\/src\/core\/.*\.tsx?$/] + exclude: [/\/packages\//]` so babel only fires for legacy-engine files; loop work in `packages/*/src/*` keeps vitest's native esbuild transform. Restored full tribunal file (was placeholder skeleton). 948 + 1 todo tests green (up from 884; +24 = restored tribunal + restored serialization suite). All 4 themed PRs rebased onto new feat/renderer-interface tip; all MERGEABLE. | b-27 (534a6c3) |
| 32 | 2026-05-29 20:00 | **B-28 indicator bench pack** — 性能 axis push (75→80). Ships `packages/core/src/__bench__/indicators.bench.ts`: full-recompute hz + µs/call for all 12 MA-family + oscillator indicators (b-9/b-10) at 10 k and 100 k bars. Real numbers locked: ZLEMA 100k = 6.7 µs/call (150 k hz); slowest Fisher Transform 100k = 3.8 ms/call (265 hz). Plus `docs/PERF_BASELINE.md` consolidating all 5 bench suites with frame-budget context (every indicator < 4 ms even at 100k full recompute; live workloads recompute only trailing window). Tribunal test updated to recognize the 5-file bench set (was hard-coded to 4). 948 + 1 todo tests green. **性能 75→80 — fourth axis to reach NORTH_STAR.** Mean 75.9 → 76.4. | b-28 |
