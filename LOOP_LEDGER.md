...
| UX | 45 | **65** | anchored zoom 10⁻¹³ px (e913fa1)；origin-shift rebaseline；B-15 keyboard shortcut registry；**+ B-17 gesture recognizer (commit b-17)** — `packages/core/src/input/gesture.ts`: 1-pointer pan + 2-pointer pinch state machine with tap/swipe terminal states, `PointerEventLike` minimal surface, panDeadzone + swipeMinVelocity + swipeVelocityWindowMs config, scale relative to pinchStart distance, pinch→tracking demotion when one finger lifts, history ring buffer for velocity smoothing; 14 tests covering tap / pan promotion / panStart-once / running dx-dy / swipe vs panEnd threshold / pinch start + scale 2× / pinch demotion / cancel / config rejection / dispose | crosshair sync + B-7 demo |
| DX | 60 | **66** | 4 publishable packages + READMEs + LICENSEs + tsconfig.build × 5 + ai-runtime；DX audit 9 BLOCKER closed 6；**BLOCKER-005 100% 关闭** — 全部 77 throw sites (b-3 + b-3b + b-3c) 现在抛 KLineChartError 带 17 个代码之一 | npm install reality + 真 dist + 剩余 3 BLOCKERs |
...
| 兼容性 | 50 | **60** | 5 包 peerDeps 合理；3 SSR 烟雾示例存在；**+ B-16 renderer tier detection (commit b-16)** — `packages/core/src/renderer-tier/`: `detectRendererTier()` 同步 WebGPU/WebGL2/Canvas2D/none cascade + 注入式 probes + 错误降级 + reason 叙事；`detectRendererTierOrThrow`、`compareRendererTier` (-1/0/1)、`isTierAtLeast` 守卫；23 tests including cascade / throw-folding / production probes don't crash | 浏览器矩阵 CI + WebGL→Canvas fallback adapter + WebGPU adapter probe |
| 美学 | 30 | **60** | **B-13 design token system + B-14 CSS-var bridge** — `packages/core/src/tokens/`: 4 token families (`ColorTokens` × 31 + palette × 10, `SpacingTokens` × 9, `TypographyTokens` × 9, `MotionTokens` × 5), 2 themes, `mergeTheme`, **+ `themeToCssVars()` / `toCssDeclarationBlock()` emit `--klc-*` custom properties (64/theme)** — any framework can apply theme via `:root` block or `style` map. 137 tests (113 + 24) covering parity / WCAG AA / merge / camel-kebab / prefix override / declaration-block | 视觉回归基线 (Playwright + 渲染) + 主题切换 demo |
