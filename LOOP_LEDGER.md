...
| 美学 | 30 | **50** | **B-13 design token system (commit b-13)** — `packages/core/src/tokens/`: 4 token families (`ColorTokens` × 31 roles + 10-color palette, `SpacingTokens`, `TypographyTokens`, `MotionTokens`), 2 paired presets (`lightTheme` / `darkTheme`), `mergeTheme()` override helper, 113 tests covering parity / hex validity / **WCAG AA contrast verified** (bull/bear ≥ 3:1 vs background; foreground text ≥ 4.5:1) / merge semantics + immutability. Industry-standard #2EBD85 / #EE4D4D failed AA on white BG — tightened to #0F8B5C / #C2363B with documented rationale | 视觉回归基线 (Playwright + 渲染) + 主题切换 demo |
