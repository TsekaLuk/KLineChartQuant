...
describe('serialize', () => {
    it('produces a SerializedChartState with schemaVersion 1 and a parseable timestamp', () => {
        const out = serialize({ label: 'Test setup', viewport: { zoomLevel: 5, visibleFrom: 0, visibleTo: 100 } })
    ...
    it('includes all four controller blocks when provided', () => {
        const out = serialize({
...
describe('deserialize', () => {
    ...
    it('NEVER eval()s — payload is data-only', () => {
        // Sanity: even a "malicious" predicate field is just data in the AST.
        ...
        // The string "process.exit(1)" round-trips as a string, not executed.
