...
function matchesQuery(def: IndicatorDefinition, q: string): boolean {
  ...
  return (
    def.label.toLowerCase().includes(needle) || def.name?.toLowerCase().includes(needle) === true
  )
}
...
export function createIndicatorSelectorController(
  ...
  function updateParams(instanceId: string, params: Record<string, ParamValue>): boolean {
    const current = active.peek()
    let found = false
