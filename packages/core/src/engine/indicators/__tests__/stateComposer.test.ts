...
function createBundle(): IndicatorSeriesBundle {
  return {
    ...
    smma: { series: [], params: {} as never },
    trima: { series: [], params: {} as never },
    zlema: { series: [], params: {} as never },
    vwma: { series: [], params: {} as never },
    alma: { series: [], params: {} as never },
    lsma: { series: [], params: {} as never },
    dma: { series: [], params: {} as never },
    gmma: { series: {}, enabledPeriods: [], params: {} as never },
...
function createDefinition(range: { min: number; max: number } | null): IndicatorMetadata {
  return {
    ...
    getRendererName: ({ paneId }) => `test_${paneId}`,
...
function createComposerDefinition(id: string, state: unknown): IndicatorMetadata {
  return {
    ...
    defaultPaneId: 'main',
    rendererFactory: vi.fn() as never,
    getRendererName: ({ paneId }) => `${id}_${paneId}`,
    getScaleRendererName: () => null,
    getPaneTitleRendererName: () => null,
    mainPane: {
      rendererName: id,
    ...
    const missingCompose = (id: string) =>
      id === 'cci'
        ? {
            ...
            indicatorType: 'other' as const,
            stateKey: '',
            defaultPaneId: '',
            rendererFactory: vi.fn() as never,
            getRendererName: ({ paneId }: { paneId: string }) => `cci_${paneId}`,
            getScaleRendererName: () => null,
            getPaneTitleRendererName: () => null,
          }
        : undefined
