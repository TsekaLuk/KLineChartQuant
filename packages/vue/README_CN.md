...
- [x] 最新价线与右轴标签样式优化
- [x] 面图元工具及渲染
- [x] **Monorepo + 多框架支持** — 拆分为 `@klinechart-quant/core`、`@klinechart-quant/vue`、`@klinechart-quant/react`、`@klinechart-quant/angular`
- [ ] React 绑定包 (`@klinechart-quant/react`)
- [ ] Angular 绑定包 (`@klinechart-quant/angular`)
- [ ] 更多高级绘图工具
- [ ] 支持分钟、多日、月、年 K 线显示
- [ ] 支持将绘制的图形转换为量化代码
...
- **v0.7.0** **Monorepo 重构 + 多框架支持** — 核心引擎提取为 `@klinechart-quant/core`；Vue 绑定迁移到 `@klinechart-quant/vue`；React 和 Angular 包已搭建。框架无关设计，支持注入式数据获取器 (`__setDataFetcher`) 和语义化配置
