/** Shared Renderer contract for Agent view state; Pi and raw IPC stay behind adapters. */
export const AGENT_UI_PROTOCOL_VERSION = 1 as const

export type AgentRunStatus =
  'idle' | 'running' | 'cancelling' | 'completed' | 'failed' | 'cancelled' | 'partial'
...
export type AgentRunUiEventInput = AgentRunUiEvent extends infer Event
  ? Event extends AgentRunUiEvent
    ? Omit<Event, 'protocolVersion' | 'runId' | 'sessionId'>
    : never
  : never
